...
async function updateUserHandler(request: NextRequest & { user: any }, { params }: { params: { id: string } }) {
  try {
    ...
    if (email) {
      ...
      `
      const emailCheckResult = await pool.query(emailCheckQuery, [email, params.id])
    ...
    if (study_program !== undefined) {
      ...
      queryParams.push(study_program)
    }
...
async function deleteUserHandler(request: NextRequest & { user: any }, { params }: { params: { id: string } }) {
  try {
    // Check if user exists
    const existingQuery = `
      SELECT id FROM users 
    ...
    if (Number.parseInt(borrowingsResult.rows[0].count) > 0) {
      return NextResponse.json({ error: "Cannot delete user with active borrowings" }, { status: 409 })
    }
