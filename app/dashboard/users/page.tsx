  ...
  const columns = [
    {
      ...
      title: "NIM",
      render: (value: string, user: User) => (user.role === "user" ? value : user.employee_id || "-"),
    },
    {
      ...
      render: (value: string, user: User) => (
        <div className="flex items-center space-x-3">
          <Avatar className="w-8 h-8">
            <AvatarFallback className="bg-blue-500 text-white text-sm">{value.charAt(0).toUpperCase()}</AvatarFallback>
          </Avatar>
          <div>
    ...
    },
    {
      ...
      render: (value: string, user: User) => (user.role === "user" ? value : user.phone || "-"),
    },
    {
      ...
      render: (value: string, user: User) => {
        ...
        return user.work_unit === "teknik-informatika" ? "Teknik Informatika" : user.work_unit || "-"
      },
    },
    {
  ...
  const handleFormSubmit = async (formData: any) => {
    try {
      if (formState.mode === "add") {
        ...
        const response = await apiClient.updateUser(formState.user.id, formData)
        setAllData((prev) => prev.map((user) => (user.id === formState.user?.id ? response.user : user)))
      }
    } catch (error) {
